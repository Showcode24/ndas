...
// ─── HELPER ────────────────────────────────────────
// _key is only required on arrays of OBJECTS, not arrays of primitives.
// Use keyedObjects() for leader[], breadcrumbItem[], ndlStat[] etc.
// Use plain arrays for string[], text[].
...
const aboutPageDocument = {
  ...
  heritage: {
    ...
    // array of text (primitives) → plain array, no _key needed
    paragraphs: [
  ...
  objectives: {
    ...
    // array of text (primitives) → plain array
    prose: [
  ...
  leadership: {
    ...
    // array of leader objects → needs _key; image omitted (upload via Studio)
    seniorLeaders: keyedObjects([
      {
    ...
    // Flat sibling string fields (not a nested object)
